...
import { SendIcon, EmojiIcon, MicrophoneIcon, MicrophoneOffIcon, VideoIcon, VideoCameraOffIcon, ShieldCheckIcon, PinIcon, MuteUserIcon, BanUserIcon, CloseIcon } from '../icons/Icons';
...
const BroadcasterView: React.FC<BroadcasterViewProps> = ({ streamTitle, onEndStream, onViewProfile, showSuccessToast }) => {
  ...
  const [viewers, setViewers] = useState(mockUsers.filter(u => u.id !== 'u1')); // Start with a list of viewers
  ...
  // State for Host Tools & Moderation
  ...
  useEffect(() => {
    const viewerInterval = setInterval(() => {
      // Don't change viewer count, just simulate activity from the existing pool
    }, 5000);
    ...
    const activityInterval = setInterval(() => {
        ...
        if (activityType < 0.7) { // 70% chance of a chat message
            if (mutedUserIds.includes(randomUser.id)) return; // Muted users don't chat
  ...
  }, [bannedUserIds, mutedUserIds, viewers]);
  ...
  const handleBanUser = (userId: string) => {
    ...
    setMutedUserIds(prev => prev.filter(id => id !== userId)); // Remove from muted if they were
  ...
  const ChatBubble: React.FC<{ message: ChatMessage; user: User }> = ({ message, user }) => {
    ...
    return (
        <button onClick={handleUserClick} className={`flex items-end gap-2 p-1 text-shadow-sm animate-fade-in-up w-full text-left ${isBroadcaster ? 'justify-end' : 'justify-start'}`}>
            {!isBroadcaster ? <Avatar /> : null}
  ...
  return (
    <>
        <div className="absolute inset-0 w-full h-full bg-black text-white overflow-hidden">
            ...
            <footer className="absolute bottom-0 left-0 right-0 z-10 p-4 flex flex-col-reverse gap-2">
                {/* This is the stable part, always at the bottom */}
                <div>
                ...
                {/* This is the dynamic part. It will appear on top of the stack */}
                {pinnedMessage && (
